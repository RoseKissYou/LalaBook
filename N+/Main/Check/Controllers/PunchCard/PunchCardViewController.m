...
- (void)datePicker{
    ...
    __weak PunchCardViewController *pcVC = self;
    self.dpView.hidden = NO;
    self.btnView.hidden = self.dpView.hidden;
    self.dpView.returnStr = ^(NSString *str){
        NSArray *strArr = [str componentsSeparatedByString:@" "];
        pcVC.indexMonth = [strArr[1] integerValue] - 1;
        label.text = [NSString stringWithFormat:@"%@年%@月",strArr[0],strArr[1]];
        pcVC.currentYear = [NSString stringWithFormat:@"%@年",strArr[0]];
        pcVC.selectYear = strArr[0];
        pcVC.selectMonth = strArr[1];
        NSString *autoidStr = [(strArr[0]) stringByAppendingString:(strArr[1])];
        NSLog(@"%@",autoidStr);
        pcVC.param.autoid = autoidStr;
    };
}
